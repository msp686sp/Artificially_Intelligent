      ...
      try {
        ...
        sock.onmessage = (msg: MessageEvent) => {
          ...
          if (parsed.type === "refresh.progress") {
            if (typeof parsed.payload.percent === "number") {
              setProgress(Math.min(1, Math.max(0, parsed.payload.percent / 100)));
            }
            if (parsed.payload.step) setStep(parsed.payload.step);
            if (parsed.payload.eta_seconds !== undefined) {
              setEtaSeconds(parsed.payload.eta_seconds ?? null);
            }
          } else if (parsed.type === "refresh.complete") {
            setProgress(1);
            setState("completed");
            callbacksRef.current.onComplete?.(parsed.payload.rows_loaded);
            closeSocket();
          } else if (parsed.type === "refresh.error") {
            const errMsg = parsed.payload.error ?? "Refresh failed.";
            setError(errMsg);
            setState("errored");
